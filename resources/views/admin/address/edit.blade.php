    ...
    <div class="col-md-12 grid-margin stretch-card">
      <div class="card">
        <div class="card-body">
          ...
          <form class="forms-sample" method="post" action="{{ url('admin/address/edit/'. $address->id) }}">
            @csrf
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <select name="countries_id" id="country" class="form-control" required>
                  ...
                  <option value="{{ $country->id }}" {{ ($country->id == $address->countries_id) ? 'selected' : '' }}>{{ $country->country_name }}</option>    
                  @endforeach
                </select>
              </div>
            </div>
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <select name="state_id" id="state" class="form-control" required>
                  ...
                  <option value="{{ $state->id }}" {{ ($state->id == $address->state_id) ? 'selected' :'' }}>{{ $state->state_name }}</option>    
                  @endforeach
                </select>
              </div>
            </div>
            <div class="row mb-3">
              ...
              <div class="col-sm-9">
                <select name="city_id" id="city" class="form-control" required>
                  ...
                  <option value="{{ $city->id }}" {{ ($city->id == $address->city_id) ? 'selected' : '' }}>{{ $city->city_name }}</option>    
                  @endforeach
                </select>
              </div>
            </div>
